 ...
 *    shows a freshly minted API key should not be able to load anything.
 *  - **A restrictive CSP meta tag** (`default-src 'none'`) on every page, so even a
 *    future mistake cannot exfiltrate a key.
 *  - **Everything interpolated goes through {@link escapeHtml}.** Client names,
 *    redirect URIs, key names, audit arguments and error messages all come from
 *    outside.
 */
 ...
 * means no other site can put the approval screen or an admin page in a frame.
...
/** The disclaimer every page ends with. */
export const FOOTER =
  "Unofficial. Not affiliated with StreetEasy. Searches through this deployment spend the operator's upstream requests.";
...
<title>${escapeHtml(options.title)} - stoop</title>
<style>${STYLE}</style>
</head>
<body>
<main>
<a class="wordmark" href="/">stoop</a>
${nav}
