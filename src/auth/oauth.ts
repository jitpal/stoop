 ...
 * `/.well-known/oauth-protected-resource`, validates access tokens on `/mcp`, and
 * forwards everything else (including the authorization page below) to our app.
 *
 ...
 *    through exactly the same path as an OAuth token, with `ctx.props` already
 *    populated, and the provider's own 401 carries the
 ...
 *    already carrying the `stoop_admin` cookie only has to press Approve; one without
 *    it is asked for `ADMIN_PASSWORD`, rate-limited, and is signed in on success, so
 *    the password is typed once per browser whichever page asked for it first. The
 *    form carries a CSRF token bound to a signed cookie, and the parsed `AuthRequest`
 *    travels through the form *signed*, so it cannot be swapped for one pointing at
 *    someone else's redirect URI.
 *
 * Every tool is read-only, so there is one scope, `read`, and nothing to choose on
 * the approval screen.
 */
...
import { type Actor, matchApiKey } from "./guard";
...
/** The one scope this deployment issues. */
export const SCOPES_SUPPORTED = ["read"];
...
const REPO_URL = "https://github.com/jitpal/stoop";
 ...
 *
 * `resourceMetadata.resource` is deliberately **not** pinned: the canonical origin is
 * a per-deployment var (`PUBLIC_BASE_URL`) and workers.dev hostnames differ per
 * account. Leaving it unset makes the provider derive the resource from the request.
 */
export function createOAuthProvider(handler: ExportedHandler<Env>): OAuthProvider {
  return new OAuthProvider<Env>({
    apiRoute: [...API_ROUTES],
    apiHandler: handler as NonNullable<OAuthProviderOptions<Env>["apiHandler"]>,
    defaultHandler: handler as OAuthProviderOptions<Env>["defaultHandler"],
    ...
    resourceMetadata: {
      ...
      resource_name: "stoop (unofficial NYC apartment search)",
      bearer_methods_supported: ["header"],
    },
    /**
     * Accepts an API key on `/mcp`, so the handler sees one uniform `ctx.props`.
     * `null` falls through to the provider's own `invalid_token` 401, which already
     * carries the challenge header.
     */
    resolveExternalToken: async ({ token, env }) => {
      const actor = await matchApiKey(env as Env, token);
      return actor ? { props: actor satisfies Actor } : null;
    },
    onError: (error) => {
      console.warn("oauth: %s", error.code, { status: error.status });
    },
  });
}
...
export function oauthRoutes(): Hono<{ Bindings: Env }> {
    ...
    return approvalResponse(
      {
        ...
        csrf,
        sealed: await sealAuthRequest(key, authRequest),
  ...
  app.post(AUTHORIZE_ENDPOINT, async (c) => {
    ...
    if (!(await verifyCsrfToken(c.req.raw, c.env, CSRF_PURPOSE, submitted.csrf))) {
      return htmlResponse(expiredPage(), 403);
    }

    const authRequest = await openSealedAuthRequest(key, submitted.sealed);
    if (!authRequest) return htmlResponse(expiredPage(), 400);
    ...
    const { redirectTo } = await helpers.completeAuthorization({
      ...
      props: { kind: "oauth", name: clientName } satisfies Actor,
    });
...
function sealAuthRequest(key: string, authRequest: AuthRequest): Promise<string> {
  return signValue(
    key,
...
/** Fields the approval form submits. */
...
function approvePage(options: ApprovePageOptions, formActionSources: string[]): string {
  ...
  return page({
    ...
    formActionSources,
    subtitle:
      "Approving lets it search rentals and building records through this deployment, spending its upstream requests.",
    body: `
...
<main>
<h1>stoop</h1>
<p>Your AI agent hunts NYC apartments: rentals by neighborhood, subway stop or any place, plus each building's public record. This is one person's own copy, running on Cloudflare Workers and speaking MCP.</p>
