 ...
 *
 * `/admin/*` is the only place a human interacts with this worker: it mints and
 * revokes API keys and shows the audit log. It is gated by the single shared secret
 * `ADMIN_PASSWORD`, and the proof of that sign-in is a stateless HMAC-signed cookie
 * (`stoop_admin`, 12 hours, `HttpOnly; Secure; SameSite=Lax`), so there is no
 * server-side session store and nothing to expire or clean up. The one piece of
 ...
 * the pair before doing anything: sign-in, OAuth approval and both API key forms. A
 ...
 * either kind, authenticates `/mcp` and nothing else.
 */
...
import { type Actor, baseUrlFrom, unauthorizedResponse } from "./guard";
import {
  clearFailures,
  clientIp,
  isRateLimited,
...
export const ADMIN_COOKIE = "stoop_admin";
/** Name of the short-lived CSRF cookie every form is bound to. */
...
/**
 * Mints the signed `stoop_admin` cookie as a ready-to-send `Set-Cookie` value.
 *
...
/**
 * True when the request carries a valid, unexpired `stoop_admin` cookie that was
 ...
 *
 * The `stoop_admin` cookie is the only credential it accepts: `/admin/*` is an
...
<p class="small muted">See the README, "Deploy".</p>`,
  });
}
