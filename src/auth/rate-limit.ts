/**
 * Deliberately naive per-IP throttle for the unauthenticated endpoints: the
 * password form (`POST /oauth/authorize`) and dynamic client registration
 * (`POST /oauth/register`). Copied from weworking.
 *
 * It lives in module memory, so it is per-isolate and evaporates on eviction. That
 * is the honest trade: `ADMIN_PASSWORD` is the only thing in front of the OAuth
 * approval, and a counter that survives 10 minutes inside one isolate already turns an
