 ...
 *
 * Three things in this worker need one: the admin session cookie (`stoop_admin`,
 * 12h), the CSRF token that binds each form to the browser that loaded it
 * (`stoop_csrf`, 10 min), and the OAuth authorization request carried through the
 * approval form. Each is a small JSON payload we hand to the browser and
 * must trust on the way back, and none justifies a Durable Object round trip, so
 * they are signed with `COOKIE_SIGNING_KEY` instead of stored.
 *
